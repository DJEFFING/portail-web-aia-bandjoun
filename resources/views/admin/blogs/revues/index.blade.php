...
@section('content')
            ...
            <div class="min-height-200px">
                ...
                <div class="pd-20 card-box mb-30">
                    <div class="clearfix mb-20">
                        <div class="pull-left">
                            <h4 class="text-blue h4">Liste Revue</h4>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            @forelse ($listRevue as $revue)
                                <tr>
                                    <th scope="row">{{ $revue->id }}</th>
                                    <td>{{ $revue->titre }}</td>
                                    <td>{{ Str::limit($revue->description, 15, '...') }}</td>

                                    <td> <button href="" class="btn btn-sm btn-outline-info me-2" data-toggle="modal"
                                            data-target="{{ '#show' . $revue->id }}">
                                            <i class="dw dw-eye"></i>
                                        </button>
                                        @if (Auth::user()->getRole('Admin'))
                                            <button href="" class="btn btn-sm btn-outline-warning me-2"
                                                data-toggle="modal" data-target="{{ '#update' . $revue->id }}">
                                                <i class="dw dw-edit2"></i>
                                            </button>
                                            <button class="btn btn-sm btn-outline-danger" data-toggle="modal"
                                                data-target="{{ '#delete' . $revue->id }}">
                                                <i class="dw dw-delete-3"></i>
                                            </button>
                                        @endif
                                    </td>

                                    @include('admin.blogs.revues.show', [
                                        'id' => $revue->id,
                                        'item' => $revue,
                                    ])

                                    @include('admin.blogs.revues.update', [
                                        'id' => $revue->id,
                                        'item' => $revue,
                                    ])
                                    @include('admin.global-modal.delete-modal', [
                                        'id' => $revue->id,
                                        'url' => route('admin.revue.delete', $revue->id),
                                    ])
    ...
    @include('admin.blogs.revues.create')
@endsection
