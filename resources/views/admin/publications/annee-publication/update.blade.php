    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #e0ee17">
                <h3 class="modal-title" id="largeModalLabel">Modifier une Année</h3>
                <button type="button" style="float: right" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">X</span>
                </button>
            </div>
            <form action="{{ route('admin.anneePublication.update',$item->id) }}" method="POST">
                @csrf
                <div class="modal-body text-center">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label>Annee : </label><span class="text-danger">*</span>
                            <input type="number" value="{{ $item->annee_publication }}" min="0" name="annee_publication" class="form-control" required>
                        </div>
                    </div>
