...
@section('content')
                ...
                <div class="page-header">
                    <div class="row">
                        <div class="col-md-6 col-sm-12">
                            <div class="title">
                                <h4>Gestion Des Roles Pour les Equipes De Projet</h4>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="pd-20 card-box mb-30">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            @forelse ($listRoleEquipeProjet as $equipeProjet)
                                <tr>
                                    <th scope="row">{{ $equipeProjet->id }}</th>
                                    <td>{{ $equipeProjet->nom }}</td>
                                    <td>{{ Str::limit($equipeProjet->description, 15, '...') }}</td>

                                    <td> <button href="" class="btn btn-sm btn-outline-info me-2" data-toggle="modal"
                                            data-target="{{ '#show' . $equipeProjet->id }}">
                                            <i class="dw dw-eye"></i>
                                        </button>
                                        @if (Auth::user()->getRole('Admin'))
                                            <button href="" class="btn btn-sm btn-outline-warning me-2"
                                                data-toggle="modal" data-target="{{ '#update' . $equipeProjet->id }}">
                                                <i class="dw dw-edit2"></i>
                                            </button>
                                            <button class="btn btn-sm btn-outline-danger" data-toggle="modal"
                                                data-target="{{ '#delete' . $equipeProjet->id }}">
                                                <i class="dw dw-delete-3"></i>
                                            </button>
                                        @endif
                                    </td>

                                    @include('admin.gestion-projet.role_projets.show', [
                                        'id' => $equipeProjet->id,
                                        'item' => $equipeProjet,
                                    ])

                                    @include('admin.gestion-projet.role_projets.update', [
                                        'id' => $equipeProjet->id,
                                        'item' => $equipeProjet,
                                    ])
                                    @include('admin.global-modal.delete-modal', [
                                        'id' => $equipeProjet->id,
                                        'url' => route('admin.roleEquipeProjet.delete', $equipeProjet->id),
                                    ])
    ...
    @include('admin.gestion-projet.role_projets.create')
@endsection
